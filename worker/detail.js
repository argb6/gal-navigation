...
function notFound(nsfwFlag) {
  return new Response("页面不存在", {
    status: 404,
    headers: {
      ...SECURITY_HEADERS,
      "Content-Type": "text/plain; charset=utf-8",
      "Set-Cookie": nsfwSetCookie(nsfwFlag),
    },
  });
}
...
if (!itemKey) {
return new Response("Missing item_key", {
  status: 400,
...
if (!row) {
return notFound(nsfwFlag);
}
const isActive = Number(row.is_active);
if (isActive === 0) {
return notFound(nsfwFlag);
}
if (isActive === NAVI_IS_NSFW && nsfwFlag !== NAVI_IS_NSFW) {
return notFound(nsfwFlag);
}
...
? `https://galnavi.top/nav/detail/?item_key=${encodeURIComponent(itemKey)}`
: "https://galnavi.top/nav/detail/";
return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
<meta name="description" content="${escapeAttr(shortDesc || title + ' - GALNAVI ' + typeLabel)}">
<meta name="keywords" content="GALNAVI, ${escapeAttr(title)}, ${escapeAttr(typeLabel)}, ACG, 二次元, Galgame, 导航">
...
<meta property="og:title" content="${escapeAttr(title)} - ${escapeAttr(typeLabel)}">
<meta property="og:description" content="${escapeAttr(shortDesc || title + ' - GALNAVI ' + typeLabel)}">
...
<meta name="twitter:title" content="${escapeAttr(title)} - ${escapeAttr(typeLabel)}">
<meta name="twitter:description" content="${escapeAttr(shortDesc || title + ' - GALNAVI ' + typeLabel)}">
<meta name="twitter:image" content="https://assets.galnavi.top/icon.png">
<script type="application/ld+json">
{"@context":"https://schema.org","@type":"WebPage","name":"${escapeAttr(title)} - ${escapeAttr(typeLabel)}","url":"${escapeAttr(canonical)}","description":"${escapeAttr(shortDesc || title + ' - GALNAVI ' + typeLabel)}","isPartOf":{"@type":"WebSite","name":"GALNAVI","url":"https://galnavi.top/"}}
</script>
...
/* gd-detail — 详情页组件（highlights-banner + triple-grid + section-card） */
...
.gd-highlights {
  ...
  padding: 20px 22px;
  margin-bottom: 20px;
  transition: background 0.3s var(--gd-motion-easing-standard), border-color 0.3s var(--gd-motion-easing-standard);
}
.gd-highlights:hover {
...
.gd-highlights:hover {
  border-color: rgba(var(--gd-color-accent-rgb), 0.28);
}
.gd-highlights__label {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 14px;
...
.gd-highlights__icon {
  ...
  justify-content: center;
  font-size: var(--gd-type-title-xxl-size);
  color: var(--gd-color-accent-light);
}
.gd-highlights__icon svg {
  width: auto;
  height: 1em;
  display: block;
}
...
.gd-highlights__item {
  ...
  line-height: 1.55;
  transition: background 0.2s var(--gd-motion-easing-standard), border-color 0.2s var(--gd-motion-easing-standard), box-shadow 0.2s var(--gd-motion-easing-standard);
}
.gd-highlights__item:hover {
  background: rgba(var(--gd-color-accent-rgb), 0.15);
  border-color: rgba(var(--gd-color-accent-rgb), 0.3);
  box-shadow: 0 0 0 3px rgba(var(--gd-color-accent-rgb), 0.18);
}
.gd-highlights__dot {
  width: 6px;
  height: 6px;
  ...
  flex-shrink: 0;
  align-self: center;
  box-shadow: 0 0 0 3px rgba(var(--gd-color-accent-rgb), 0.3);
}
.gd-highlights__text {
  flex: 1;
  min-width: 0;
...
/* 详情卡片 — 背景同主站卡片；触碰发光同 gd-card（过渡丝滑） */
.gd-section-card {
  display: flex;
  flex-direction: column;
  min-height: 0;
  ...
  transition:
    ...
    filter var(--gd-motion-duration-medium1) var(--gd-motion-easing-standard);
}
.gd-section-card:hover {
  background: var(--gd-glass-bg-hover);
  border-color: var(--gd-color-border-hover);
  box-shadow: 0 0 24px rgba(var(--gd-color-accent-rgb), 0.14), inset 0 1px 0 rgba(var(--gd-color-white-rgb), 0.06);
  filter: brightness(1.05);
}
.gd-section-card:hover {
  border-color: var(--gd-color-border-accent);
}
.gd-section-card__header {
  display: flex;
  align-items: center;
  gap: 10px;
  margin-bottom: 14px;
...
.gd-section-card__icon svg {
  width: auto;
  height: 1em;
  display: block;
}

/* 预览：名称 - 图标 行（图标不被 24px 容器裁剪） */
.gd-icon-demo {
  display: inline-flex;
  align-items: center;
  gap: 8px;
...
.gd-icon-demo .gd-icon-demo__name {
  color: var(--gd-color-on-surface-variant);
  font-size: var(--gd-type-title-small-size);
}
.gd-icon-demo .gd-icon-demo__sep {
  color: var(--gd-color-on-surface-subtle);
  font-size: var(--gd-type-title-small-size);
}
.gd-section-card__title {
  font-size: var(--gd-type-title-xxl-size);
  font-weight: 550;
  color: var(--gd-color-on-surface);
  font-family: var(--gd-font-sans);
...
/* 条目行：链接 / 纯文本 */
.gd-section-card__link,
.gd-section-card__entry {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 11px 14px;
  ...
  position: relative;
  min-width: 0;
  transition: background 0.2s var(--gd-motion-easing-standard), border-color 0.2s var(--gd-motion-easing-standard), color 0.2s var(--gd-motion-easing-standard), box-shadow 0.2s var(--gd-motion-easing-standard);
}
.gd-section-card__link {
  cursor: pointer;
  font-family: var(--gd-font-sans);
}
.gd-section-card__link:hover,
.gd-section-card__entry:hover {
  background: rgba(var(--gd-color-sky-rgb), 0.15);
  border-color: rgba(var(--gd-color-sky-rgb), 0.3);
  box-shadow: 0 0 0 3px rgba(var(--gd-color-sky-rgb), 0.14);
  color: var(--gd-color-on-surface);
}
.gd-section-card__label {
  font-weight: var(--gd-weight-medium);
  color: var(--gd-color-sky);
  white-space: normal;
  word-break: break-all;
  overflow-wrap: break-word;
  min-width: 0;
}
.gd-section-card__url {
  margin-left: auto;
  ...
  max-width: 120px;
  opacity: 0;
  transition: opacity 0.2s var(--gd-motion-easing-standard);
}
.gd-section-card__link:hover .gd-section-card__url {
  opacity: 1;
}
.gd-section-card__entry {
  cursor: default;
...
.gd-section-card__entry .gd-section-card__label {
  color: var(--gd-color-on-surface-subtle);
...
@media (hover: none) {
  ...
  .gd-section-card:active {
    box-shadow: 0 0 24px rgba(var(--gd-color-accent-rgb), 0.2), inset 0 1px 0 rgba(var(--gd-color-white-rgb), 0.06);
    background: var(--gd-glass-bg-hover);
    border-color: var(--gd-color-border-accent);
  }
}
...
@media (prefers-reduced-motion: reduce) {
  ...
  .gd-highlights__item:hover { transform: none; }
}
...
#detailSkeleton .gd-skeleton__header {
  display: flex;
  flex-direction: column;
  margin-bottom: 22px;
}
#detailSkeleton .gd-skeleton__title {
  width: 46%;
  height: 54px;
  border-radius: 12px;
  margin-bottom: 15px;
}
#detailSkeleton .gd-skeleton__desc {
  width: 68%;
  height: 25px;
  border-radius: 7px;
  margin-bottom: 15px;
}
#detailSkeleton .gd-skeleton__tags {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}
#detailSkeleton .gd-skeleton__tag {
  width: 74px;
  height: 29px;
  border-radius: 9999px;
}
#detailSkeleton .gd-skeleton__banner {
  padding: 20px 22px;
  border-radius: var(--gd-shape-corner-medium);
  background: var(--gd-glass-bg);
  border: 1px solid var(--gd-glass-border);
...
#detailSkeleton .gd-skeleton__banner-head {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 6px;
}
#detailSkeleton .gd-skeleton__banner-icon {
  width: 18px;
  height: 18px;
  border-radius: 6px;
  flex-shrink: 0;
}
#detailSkeleton .gd-skeleton__banner-label {
  width: 90px;
  height: 18px;
  border-radius: 8px;
}
#detailSkeleton .gd-skeleton__banner-item {
  height: 45px;
  border-radius: 12px;
}
#detailSkeleton .gd-skeleton__banner .gd-skeleton__card-line {
  display: none;
}
#detailSkeleton .gd-skeleton__grid {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 16px;
  align-items: stretch;
}
#detailSkeleton .gd-skeleton__card {
  display: flex;
  flex-direction: column;
  gap: 12px;
  padding: 18px;
...
#detailSkeleton .gd-skeleton__card-head {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 2px;
}
#detailSkeleton .gd-skeleton__card-icon {
  width: 18px;
  height: 18px;
  border-radius: 6px;
  flex-shrink: 0;
...
#detailSkeleton .gd-skeleton__card-size {
  width: 90px;
  height: 18px;
  border-radius: 8px;
}
#detailSkeleton .gd-skeleton__card-line {
  width: 100%;
  height: 45px;
  border-radius: 12px;
}
#detailSkeleton .gd-skeleton__card-line--sm { width: 70%; height: 45px; }
#detailSkeleton .gd-skeleton__block {
  position: relative;
  overflow: hidden;
...
#detailSkeleton .gd-skeleton__block::after {
  content: "";
  position: absolute;
  inset: 0;
...
@media (max-width: 768px) {
  #detailSkeleton .gd-skeleton__grid { grid-template-columns: 1fr; }
}
.gd-footer { margin-top: auto; padding-top: 24px; }
@keyframes fadeInUp {
...
@media (prefers-reduced-motion: reduce) {
  ...
  .tags-row,
  .gd-highlights,
  .gd-section-card-grid .gd-section-card,
  .gd-footer {
    animation: none;
  }
}
@media (max-width: 768px) {
  body.gd-detail { padding: 58px 16px 40px; font-size: var(--gd-type-body-large-size); line-height: 1.7; }
  .gd-detail__container { display: flex; flex-direction: column; }
  .gd-detail__header { text-align: center; margin-bottom: 18px; }
  .gd-detail__header-top { flex-direction: column; align-items: center; gap: 8px; }
  .gd-detail__header-top .gd-brand__title { font-size: 32px; }
  .gd-detail__intro { font-size: var(--gd-type-body-large-size); text-align: center; max-width: none; line-height: 1.65; }
  .tags-row { justify-content: center; margin-top: 12px; gap: 8px; }
  .tag { font-size: var(--gd-type-label-large-size); padding: 6px 12px; }
  .gd-footer { margin-top: auto; }
}
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
...
<a href="https://galnavi.top/sitemap.xml">sitemap.xml</a><span class="gd-footer__sep" aria-hidden="true">|</span><a href="https://galnavi.top/robots.txt">robots.txt</a><span class="gd-footer__sep" aria-hidden="true">|</span><a href="mailto:feedback@galnavi.top">联系站长</a><span class="gd-footer__sep" aria-hidden="true">|</span><a href="https://galnavi.top/nav/donate/">赞助本站</a><span class="gd-footer__sep" aria-hidden="true">|</span><a href="https://galnavi.top/nav/friend/">申请友链</a><span class="gd-footer__sep" aria-hidden="true">|</span><a href="https://galnavi.top/status/">站点状态</a>
</nav>
...
.map(
(t, i) =>
`<a class="tag ${TAG_COLORS[i % TAG_COLORS.length]}" href="https://galnavi.top/nav/?q=${encodeURIComponent(t)}">${escapeHtml(t)}</a>`
)
.join("\n");
