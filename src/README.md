...
为避免样式互相污染，组件 class 统一使用 `gd-` 前缀，设计变量统一使用 `--gd-` 前缀。组件内部也避免使用 `.title`、`.active` 这类过宽的选择器。玻璃边界与动效约定见 `docs/tokens.md`、`docs/coding-style.md`。
...
│  │  └─ gd-preview.js         打包后的预览脚本（由 esbuild 入口（内部） 构建）
│  │
│  ├─ foundation/ 基础
│  │  ├─ tokens/               tokens.css（--gd-* 变量，唯一换肤入口）+ gd-glass.css
│  │  ├─ actions/              gd-button.css · gd-link.css · gd-icon-button.css
│  │  ├─ brand/                gd-brand.css
│  │  ├─ layout/               gd-layout.css + gd-footer.css
│  │  └─ accessibility/        gd-a11y.css（跳过链接 + forced-colors）
│  │
│  ├─ navigation/ 导航
...
│  │
│  ├─ feedback/ 反馈
│  │  ├─ modal/（含 age-gate 变体）  toast/  tooltip/  skeleton/
│  │
│  ├─ extend/ 页面扩展（按来源 Worker 页面归类）
│  │  ├─ overview/   总览页壳层（虚线分割、索引）✓
│  │  ├─ websearch/  filter-bar + 纳普彩蛋 ✓
│  │  ├─ home/       发布页（index.js）✓
│  │  ├─ about/      关于页               ✓
│  │  ├─ help/       帮助页               ✓
│  │  ├─ donate/     捐献页              ✓
...
└─ worker/                    服务端（不打包进浏览器）
   ...
   └─ shared/                   constants.js · security.js · seo.js
```
...
维护文档在项目根 `docs/`，版本与状态见 `docs/gd.config.json`。
...
- `initGdAgeGate("#ageGate", { storageKey, onDeny })`：年龄门，已确认用户直接放行；
...
[`preview/index.html`](./preview/index.html) 是组件总览页。页面引用打包脚本 [`preview/gd-preview.js`](./preview/gd-preview.js)（普通 `<script>`，非 ES Module），因此直接双击 `file://` 或通过静态 HTTP 服务打开均可。预览时应检查桌面和窄屏布局，并实际操作搜索、抽屉、弹窗、轮播、年龄门和 Toast。

修改任一组件 JS 后需要重新打包 `gd-preview.js`（打包入口在 `esbuild 入口（内部）`，配置在项目根 `esbuild.config.js`）：
...
`src/` 负责浏览器侧的 CSS、预览页和少量通用行为。Worker 页面仍然负责拼 HTML、查询数据、过滤、跳转和页面级决策；服务端安全、SEO 和业务常量继续放在 `worker/shared/`，不打包进浏览器。

接入时可以按页面逐步替换旧 class。先替换 `gd-button`、`gd-card` 等外观，再接入 `<gd-modal>`、`<gd-navbar>`、`<gd-search>`、`initGdAgeGate`，这样每一步都容易回退和检查。

整体架构见 [`ARCHITECTURE.md`](../ARCHITECTURE.md)，Worker 组件盘点见 [`组件盘点（内部）`](../组件盘点（内部）)。
...
| [`docs/tokens.md`](../docs/tokens.md) | 设计变量（换肤入口） |
| [`docs/components.md`](../docs/components.md) | 组件索引（核心文件） |
| [`docs/usage.md`](../docs/usage.md) | 使用规范与开发流程 |
| [`docs/migration.md`](../docs/migration.md) | 迁移记录 |
| [`docs/coding-style.md`](../docs/coding-style.md) | 编码规范 |
| [`docs/examples/`](../docs/examples/) | 使用示例（card/modal/navbar/search/carousel/toast） |
