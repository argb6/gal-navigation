...
- **class**：`gd-search`、`gd-search__box/icon/input/clear/help`、`gd-search--toolbar/--group`
- **事件**：input 显清除、clear 回焦；仅带 `expandable` 时聚焦变宽（顶栏搜索固定 300px，不要加 `expandable`）
- **搜索规则问号（可选）**：输入框右侧 `.gd-search__help`；悬停 / 键盘焦点显示 `.gd-search__help-tip`（`gd-tooltip`）。自定义元素加 `help` 属性即可；`help-text` 可改文案。主站顶栏已接入。
- **group 变体规范（圣器殿堂搜索框）**：
...
| `gd-skeleton` | `feedback/skeleton/` | 骨架屏（`--card` / `--hero` / `--detail` / `--item`） | 纯 CSS |；`--card` 对齐主站卡（多列 390×100，仅一列宽 100%）
...
| `gd-below-nav` / `gd-rec-tags` | `extend/websearch/` | 通知条 + 推荐标签 | 与 `.gd-navbar--led` 共用 `--gd-chrome-bar-bg` |
