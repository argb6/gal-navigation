...
| palace.toml | D1 `group1` | `fbf5b2ca-8a74-464b-ab41-0d990c43b64e` |
| donate.toml | DONATE_KV | `93deddcc283b41e7962bf60929fb269` |

`about.toml` / `help.toml` / `galnavi.toml` / `error.toml` 现网无 D1。`error.toml` 无 STATUS_KV（只有 service binding，且没有 status）。仓库里没有 `d1-rest.toml`、`shenmo.toml`、`moho.toml`。
...
| `wrangler/shenmo.toml` | 已不在仓库，用 `palace.toml` |
| `worker/galnavi.js` / `worker/shenmo.js` | 已不在仓库根 worker，现网是 `worker/new/` |
...
| `kb/` | 空目录 |
| `source/` | **保留**：`worker/new` 阅读副本（十页齐全；token 明文已掏空） |
...
| 路径 | 原因 |
|------|------|
| （根目录旧稿已删）`gd-architecture.md` / `worker-component-report.md` / `galnavi升级计划.md` | 改看 `docs/architecture.md`、`docs/components.md`、`docs/decisions/` |
...
├── eslint.config.mjs
├── src/                    gd 组件库
├── source/                 worker/new 阅读副本（无 token 明文）
