...
- `gd-orb` 圆钮可在页面内拖动，松手后位置记在 `galnavi-orb-pos`；移动不到约 6 像素仍算点击。靠近上沿或左沿时菜单翻到下方或右侧。预览 `.gd-orb--demo` 不启用拖动
...
- 主站卡片去卡面标签与双按钮，点击整卡弹出 `gd-modal` 展示详情（标签 / 介绍详情 / 链接直达）；弹窗标签全部显示
- 主站卡弹窗关闭钮改为圆形玻璃按钮
- 搜索帮助问号固定在输入框右侧；说明气泡改为绝对定位，手机端不再被挤到中间
- 顶栏通知改为闲聊群、帮助文档、更多标签、了解本站；跑马灯按整段复制，位移用 `--gd-notice-led-shift`，无限循环不再闪
- 主站预览去掉推荐标签条（`.gd-rec-tags` 样式仍保留）
- `gd-orb` 快捷入口只留标签、仓库、弹窗、殿堂；按钮图标改为五角星
- 页脚链接改为：帮助文档、关于本站、联系站长、申请友链、站点状态（去掉 sitemap / robots / 赞助本站）

### 完善

- 主站卡宽 min(400px, 100%)，高按字号行盒固定（title 16×1.4 + 简介 14×1.65×3），不再固定 212px / 420px
- `gd-skeleton--card` 同步：宽 min(400px, 100%)；`--title` / `--sub` 条高度对齐；去掉标签和双按钮占位，只留图标、标题、简介
