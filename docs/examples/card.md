...
| 场景 | class |
|---|---|
| 主站卡片 | `gd-card gd-card--general`（可点加 `gd-card--link`）；宽 min(400px,100%)，高按字号行盒固定（简介最多 3 行） |
| 友链整卡 | `gd-card gd-card--link gd-card--friend` |
| 条目卡（殿堂） | `gd-card gd-card--item`（+ `gd-card--item--demonic/immortal`；默认金色 divine） |

## 殿堂条目卡

列表行，主站卡现已 `height 按 title/sub 字号行盒计算固定`（宽仍 min(400px,100%)）；条目卡仍须显式。组件库已写 `width: auto; height 按 title/sub 字号行盒计算固定`，Worker 内联副本必须带上，否则序号和按钮还在、游戏名会被挤没。
